...
function seg = chenvese(I,mask,num_iter,mu,method,usermask)
...
%   auto mask settings
  if ischar(mask)
      ...
      if s<1
          ...
          disp('size of mask after resizing')
          size(mask)
      end
...
switch lower(method)
    case {'chan','vector'}
        ...
        mask = mask(:,:,1);
%         bwdist(mask)
%         bwdist(mask)-bwdist(1-mask)
%         bwdist(mask)-bwdist(1-mask)+im2double(mask)-.5
